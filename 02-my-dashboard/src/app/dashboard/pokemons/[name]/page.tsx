...
//   return static151Pokemon.map((id) => ({ id: id }));
    ...
    const pokemolList = await fetch("https://pokeapi.co/api/v2/pokemon?limit=151");
    const { results } = await pokemolList.json();

    return results.map((pokemon: { name: string }) => ({ name: pokemon.name }));
}
